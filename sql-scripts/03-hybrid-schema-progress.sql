...
CREATE TABLE IF NOT EXISTS user_progress (
  ...
  accuracy_rate DECIMAL(5, 2) GENERATED ALWAYS AS (
    CASE
      WHEN total_diagnoses > 0 THEN (
        correct_diagnoses::DECIMAL / total_diagnoses * 100
      )
      ELSE 0
    END
  ) STORED,
...
CREATE TABLE IF NOT EXISTS journey_sessions (
  ...
  user_id BIGINT NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  session_number INT NOT NULL CHECK (
    session_number BETWEEN 1 AND 12
  ),
  session_phase TEXT CHECK (
    session_phase IN (
      'psychoeducation',
      'formulation',
      'intervention',
  ...
  patient_improvement_score INT CHECK (
    patient_improvement_score BETWEEN 0 AND 100
  ),
  patient_symptoms_change TEXT,
  therapist_performance_score INT CHECK (
    therapist_performance_score BETWEEN 0 AND 100
  ),
...
CREATE TABLE IF NOT EXISTS user_achievements (
  ...
  badge_tier TEXT CHECK (
    badge_tier IN ('bronze', 'silver', 'gold', 'platinum')
  ),
