...
async function generateCase(disorder, level) {
...
async function populateAnxiety() {
  console.log('😰 POPULAÇÃO DE CASOS DE ANSIEDADE');
  console.log('===================================\n');
  console.log('📊 15 casos (5 transtornos × 3 níveis)\n');
  ...
  for (const level of LEVELS) {
    ...
    for (let i = 0; i < ANXIETY_DISORDERS.length; i++) {
      const disorder = ANXIETY_DISORDERS[i];
      process.stdout.write(`  ${i + 1}/5 ${disorder.substring(0, 35)}... `);
      
      try {
        const result = await generateCase(disorder, level);
        if (result) {
          console.log('✅');
          totalGenerated++;
        } else {
