...
function calculateLevelAndTitle(totalCognits) {
  ...
  if (totalCognits <= 150) {
    ...
    if (totalCognits < 50) {
      level = 1; title = 'Estudante de Lente';
    } else if (totalCognits < 100) {
      level = 2; title = 'Observador Clínico';
    } else {
      level = 3; title = 'Apontador de Sintomas';
    }
  } else if (totalCognits <= 500) {
    // Intermediário: 151-500 cognits
    if (totalCognits < 266) {
      level = 4; title = 'Decodificador Diagnóstico';
    } else if (totalCognits < 383) {
      level = 5; title = 'Mapeador de Comorbidades';
    } else {
      level = 6; title = 'Construtor de Linha do Tempo';
    }
  } else if (totalCognits <= 1200) {
    // Avançado: 501-1200 cognits
    if (totalCognits < 733) {
      level = 7; title = 'Lente Rápida';
    } else if (totalCognits < 966) {
      level = 8; title = 'Escultor de Conceituação';
    } else {
      level = 9; title = 'Terapeuta de Estratégia';
    }
  } else {
    // Maestria: 1201+ cognits
    if (totalCognits < 2000) {
      level = 10; title = 'Arquiteto Cognitivo';
    } else if (totalCognits < 3000) {
      level = 11; title = 'Mentor de Diagnóstico';
    } else {
      level = 12; title = 'Clínico de Alta Performance';
    }
  }
...
router.get('/summary', authenticateRequest, async (req, res) => {
  try {
    ...
    if (totalCognits < 151) {
      nextLevelAt = 151;
    } else if (totalCognits < 501) {
      nextLevelAt = 501;
    } else if (totalCognits < 1201) {
      nextLevelAt = 1201;
    } else {
      nextLevelAt = totalCognits + 500;
    } // Maestria continua crescendo
