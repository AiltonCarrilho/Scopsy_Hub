...
// ============================================
// CARREGAR SESSÕES COM ESTRUTURA COMPLETA
// ============================================
const fs = require('fs');
...
router.get('/:journeyId/sessions', authenticateRequest, async (req, res) => {
  try {
    ...
    logger.info('Carregando sessões estruturadas', { journeyId, userId });
    ...
    if (cached && (Date.now() - cached.timestamp < CACHE_TTL)) {
      return res.json({
        ...
        metadata: { cached: true, cache_ttl_seconds: CACHE_TTL / 1000 }
      });
    }
    ...
    const ranges = [
      `journey-${journeyId}-sessions-1-3-intermediate.json`,
      `journey-${journeyId}-sessions-4-6-intermediate.json`,
      `journey-${journeyId}-sessions-7-9-intermediate.json`,
      `journey-${journeyId}-sessions-10-12-intermediate.json`
    ];
    ...
    // 4. Montar resposta
    const journey = journeyMetadata || { id: journeyId, title: `Journey ${journeyId}`, total_sessions: 12 };
    const response = {
      success: true,
      journey,
      sessions: sessions.sort((a, b) => a.session_number - b.session_number),
      metadata: { cached: false, cache_ttl_seconds: CACHE_TTL / 1000 }
    };

    // 5. Cache
    sessionCache.set(cacheKey, { journey, sessions: response.sessions, timestamp: Date.now() });

    res.json(response);
  } catch (error) {
    logger.error('Erro ao carregar sessões', { error: error.message });
