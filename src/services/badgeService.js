...
async function checkAndAwardBadges(userId) {
  ...
  try {
    ...
    for (const badge of definitions) {
      ...
      // --- STREAKS ---
      case 'streak-3':
        if (user.longest_streak >= 3 || user.current_streak >= 3) {
          earnedIt = true;
        }
        break;
      case 'streak-7':
        if (user.longest_streak >= 7 || user.current_streak >= 7) {
          earnedIt = true;
        }
        break;
      case 'streak-30':
        if (user.longest_streak >= 30 || user.current_streak >= 30) {
          earnedIt = true;
        }
        break;
      ...
      case 'pioneer': {
        ...
        if (creation < new Date('2025-01-01')) {
          earnedIt = true;
        }
        break;
      }
      ...
      case 'analyst-expert': { // 50 casos
        ...
        if (badge.slug === 'analyst-novice' && caseCount >= 5) {
          earnedIt = true;
        } else if (badge.slug === 'analyst-expert' && caseCount >= 50) {
          earnedIt = true;
        }
        break;
      }
      ...
      case 'sharp-eye': { // 10 diagnósticos corretos
        ...
        // Vou pular por enquanto se não tiver tabela fácil.
        break;
      }
      }
...
async function countActivities(userId, type, _mustBeSuccess = false) {
