...
const CATEGORIES = [
  ...
  },
  {
    ...
    disorders: [
      ...
      'Dependência de Múltiplas Substâncias'
    ],
    casesPerLevel: 4
  }
];
...
async function populateAllCategories() {
  console.log('🗂️ POPULAÇÃO COMPLETA DE CATEGORIAS');
  console.log('=====================================\n');
  ...
  for (const category of CATEGORIES) {
    ...
    console.log(`   Casos por nível: ${category.casesPerLevel}`);
    console.log(`   Total esperado: ${category.disorders.length * LEVELS.length}\n`);

    for (const level of LEVELS) {
      console.log(`  📊 Nível: ${level}`);
      
      // Distribuir disorders pelos casos deste nível
      const disordersToUse = [];
      for (let i = 0; i < category.casesPerLevel; i++) {
        disordersToUse.push(category.disorders[i % category.disorders.length]);
      }

      for (let i = 0; i < disordersToUse.length; i++) {
        const disorder = disordersToUse[i];
        process.stdout.write(`     ${i + 1}/${disordersToUse.length} ${disorder.substring(0, 30)}... `);
...
populateAllCategories().catch(err => {
