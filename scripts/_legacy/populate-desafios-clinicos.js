/**
 * POPULATE DESAFIOS CLÍNICOS
 * Popula banco com casos de alta qualidade seguindo formato Scopsy
 */
...
// Gerar TAG temporal: 2026.01.15.1432
...
// CASOS DE ALTA QUALIDADE
const casos = [
    // CASO 1: Resistência Técnica (BASIC) - Fornecido pelo usuário
    {
        tag: "2026.01.15.1432",
...
// Função para inserir casos no Supabase
async function populateCases() {
    console.log('🚀 Iniciando população de Desafios Clínicos...\n');

    for (const caso of casos) {
        const caseData = {
            case_title: `${caso.context.client_name} - ${caso.moment_type.replace(/_/g, ' ')}`,
            disorder: caso.context.diagnosis,
            difficulty_level: caso.difficulty_level,
            moment_type: caso.moment_type,
            case_content: caso, // JSON completo
            vignette: caso.critical_moment.dialogue,
            status: 'active',
            created_by: 'sistema',
        ...
        if (error) {
            console.error(`❌ Erro ao inserir caso ${caso.tag}:`, error.message);
        } else {
            console.log(`✅ Caso inserido: ${caso.tag} - ${caso.context.client_name} (${caso.difficulty_level})`);
        }
    }

    console.log('\n🎉 População concluída!');
    console.log(`📊 Total de casos inseridos: ${casos.length}`);
}
